...
void CProgressBar::Progress_UpdateY()
{
	if (m_iPreValue != m_iCulValue || m_iPreMaxValue != m_iCulMaxValue)
	{
		_float fRatio{};
		...
		_float fOffsetY = (1.f - fRatio) * m_fSizeY * 0.5f;
		m_pTransformCom->Scaling(m_fSizeX, m_fSizeY, 1.f);

		m_pTransformCom->Scaling(m_fSizeX, m_fSizeY * fRatio, m_fZ);
		m_pTransformCom->Set_State(STATE::POSITION, _float3{ m_vWorldPos.x - m_iWinSizeX * 0.5f,(-m_vWorldPos.y + m_iWinSizeY * 0.5f) -fOffsetY, m_vWorldPos.z });
