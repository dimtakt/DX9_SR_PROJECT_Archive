...
{ 
	m_iCulMaxValue = CStat_Manager::GetInstance()->Get_CurStats()[ENUM_CLASS(STAT_INFO::MAXHP)];
	m_iCulValue = CStat_Manager::GetInstance()->Get_CurStats()[ENUM_CLASS(STAT_INFO::CULHP)];
	Progress_UpdateX();
...
void CHp_Player::FontRender_Switch()
{
	if (m_isFontRender)
