...
HRESULT CMeteor::Initialize(void* pArg)
{
    ...
    m_pGameInstance->Subscribe(ENUM_CLASS(EVENT_TYPE::METEOR), this);
    return S_OK;
}
...
void CMeteor::Update(_float fTimeDelta)
{
    if (m_bMeteor)
    {
        m_fMeteorTimer += fTimeDelta;

        if (m_fMeteorTimer >= 1.f && m_iMeteorCount > 0)
        {
            Create_Meteor();
            m_fMeteorTimer = 0.f;
            m_iMeteorCount--;
...
void CMeteor::OnEvent(_uint iTypeindex, const EVENTDATA* pData)         
{
    if (static_cast<EVENT_TYPE>(iTypeindex) == EVENT_TYPE::METEOR)
    {
        ...
        if (m_fCurrentCoolTime <= 0.f)
        {
            ...
            // --- 랜덤 위치 잡아주기
            _float fRandom = 6.f;       // 중심 기준 최대 랜덤 오프셋 
...
void CMeteor::Free()
{
    m_pGameInstance->Unsubscribe(ENUM_CLASS(EVENT_TYPE::METEOR), this);
