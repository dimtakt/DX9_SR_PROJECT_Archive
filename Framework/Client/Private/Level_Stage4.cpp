...
HRESULT CLevel_Stage4::Ready_Layer_Room(const _wstring& strLayerTag)
{
	...
	_int iCount = 0;
	_int iEventCheck = 0;
	_int iEventRoomType = 1; // 타입 전달, 1이면 shop, 2면 hp, 3 EXP, 4 아티팩트, 5 석판.. 구조 추후 바꿔줄 예정 급하게 막 짜둠. 
	...
	while (iCount < iIndex)
	{
		...
		{
			pRoom->Load_From_File(ENUM_CLASS(LEVEL::LEVEL_STAGE2), strLayerTag, TEXT("../../data/Stage3_Event%d.txt"), iEventCheck, RoomX, RoomZ, static_cast<ROOM_INFO>(iEventRoomType));			// 이벤트 룸 로드
			iEventCheck++;
			iEventRoomType++;
			iCount--;
