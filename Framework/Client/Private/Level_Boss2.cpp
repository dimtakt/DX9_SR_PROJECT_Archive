...
HRESULT CLevel_Boss2::Initialize()
{
	...
	m_pGameInstance->PlayBGM(L"placeDungeonBossBadLand_Loop_New.wav", g_fBGMVolume - 0.9f);
...
HRESULT CLevel_Boss2::Ready_Light(const _wstring& strLayerTag)
{
	CPoint::POINTDESC desc{};
	desc.pTargetTransform = dynamic_cast<CTransform*>(m_pGameInstance->Get_GameObject(ENUM_CLASS(LEVEL::LEVEL_BOSS2), TEXT("Layer_Player"))->Find_Component(TEXT("Com_Transform")));
	...
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::LEVEL_BOSS2), strLayerTag,
...
HRESULT CLevel_Boss2::Ready_Layer_Camera(const _wstring& strLayerTag)
{
	CCamera_Follow::CAMERAFOLDESC desc{};
	desc.iLayerIndex = ENUM_CLASS(LEVEL::LEVEL_BOSS2);

	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::LEVEL_BOSS2), strLayerTag,
		ENUM_CLASS(LEVEL::LEVEL_BOSS2), TEXT("Prototype_GameObject_Camera_Follow"), &desc)))
		return E_FAIL;

	//if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::LEVEL_BOSS2), strLayerTag,
	//	ENUM_CLASS(LEVEL::LEVEL_BOSS2), TEXT("Prototype_GameObject_Camera_Mouse"))))
	//	return E_FAIL;
...
HRESULT CLevel_Boss2::Ready_Layer_BackGround(const _wstring& strLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::LEVEL_BOSS2), strLayerTag,
		ENUM_CLASS(LEVEL::LEVEL_BOSS2), TEXT("Prototype_GameObject_Sky"))))
		return E_FAIL;

	return S_OK;
}

HRESULT CLevel_Boss2::Ready_Layer_Player(const _wstring& strLayerTag)
{
	CDagger::DAGGERDESC DaggerDesc{};
	DaggerDesc.iLayerIndex = ENUM_CLASS(LEVEL::LEVEL_BOSS2);

	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::LEVEL_BOSS2), TEXT("Layer_Weapon"),
		ENUM_CLASS(LEVEL::LEVEL_STATIC), TEXT("Prototype_GameObject_Weapon_Dagger"), &DaggerDesc)))
		return E_FAIL;

	CPlayer::PLAYERDESC PlayerDesc{};
	PlayerDesc.iLayerIndex = ENUM_CLASS(LEVEL::LEVEL_BOSS2);

	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::LEVEL_BOSS2), TEXT("Layer_Player"),
...
HRESULT CLevel_Boss2::Ready_Layer_UI(const _wstring& strLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::LEVEL_BOSS2), TEXT("Layer_Hud_States"),
	...
	if (FAILED(m_pGameInstance->Add_GameObject_ToLayer(ENUM_CLASS(LEVEL::LEVEL_TOWN), TEXT("Layer_Ending"),
...
HRESULT CLevel_Boss2::Ready_Layer_Room(const _wstring& strLayerTag)
{
	...
	CRoom_Manager::GetInstance()->Create_SpecialRoom(LEVEL::LEVEL_BOSS2); // 보스룸의 인덱스는 무조건 고정적으로 넣어주고자 함. 랜덤X

	while (iCount < iIndex)
	{

		pRoom = dynamic_cast<CRoom*>(m_pGameInstance->Clone_Prototype(PROTOTYPE::GAMEOBJECT, ENUM_CLASS(LEVEL::LEVEL_BOSS2), TEXT("Prototype_GameObject_Room")));
		...
		if (iCount == 0)
		{
			CPlayer* pPlayer = dynamic_cast<CPlayer*>(m_pGameInstance->Get_GameObject(ENUM_CLASS(LEVEL::LEVEL_BOSS2), TEXT("Layer_Player")));
